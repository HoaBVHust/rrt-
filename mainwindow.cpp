        ...
        if (q) {
            ...
            if (rrt->distance(q->position, qNearest->position) > rrt->step_size) {
                ...
                if (!rrt->obstacles->isSegmentInObstacle(newConfig, qNearest->position)) {
                    ...
                    Node *qMinCost = rrt->minCost(newConfig);
                    rrt->add(qMinCost, qNew);
                    for(int i = 0; i < (int)rrt->nodes.size(); i++) {
                        if(rrt->distance(qNew->position, rrt->nodes[i]->position)<3*rrt->step_size){
                            if (qNew->cost+rrt->distance(rrt->nodes[i]->position,qNew->position)<rrt->nodes[i]->cost) {
                                rrt->nodes[i]->parent=qNew;
                                rrt->nodes[i]->cost = qNew->cost+rrt->distance(rrt->nodes[i]->position,qNew->position);
                            }
                        }
                    }
                }
